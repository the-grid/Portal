...
class BlockSpec: QuickSpec {
    override func spec() {
        ...
        let html = "<p>Grr</p>"
        let id = "a486de43-c7e9-4ea1-8e68-2cec66270ce6"
        let item = "0bb09c2c-aacc-41e6-a9d5-9a3929019745"
        let type = "text"
        ...
        describe("decoding") {
            it("should produce a Block") {
                guard let decoded = Block.decode(json).value else {
                    return XCTFail("Unable to decode JSON: \(json)")
                }
                expect(decoded).to(equal(block))
            }
        }
